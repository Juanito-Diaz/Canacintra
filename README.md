...
MySQL Server (se recomienda usar XAMPP o WampServer para una configuración rápida en Windows).
Visual Studio Build Tools / Compilador C++ (opcional, requerido en algunos entornos Windows para compilar el conector mysqlclient).
...
Con el entorno virtual activado, instala las librerías necesarias mediante el archivo 
...
Inicia tus servicios de MySQL y Apache en el panel de XAMPP.
Accede a phpMyAdmin (http://localhost/phpmyadmin/).
Crea una base de datos nueva llamada exactamente:
sql
canancintra
(Nota: Asegúrate de respetar la ortografía canancintra, ya que es la configurada en los settings del proyecto).
El usuario configurado por defecto es root con contraseña root en el puerto 3306. Si tu configuración local de XAMPP difiere (por ejemplo, sin contraseña), puedes ajustarla en el archivo 
...
El Portal de Noticias e interfaz pública: http://127.0.0.1:8000/
El Panel de Administración Personalizado: http://127.0.0.1:8000/dashboard/
El Administrador nativo de Django: http://127.0.0.1:8000/admin/
...
canacintra_project/
: Configuración global de la aplicación Django (URLs principales, WSGI/ASGI y archivo de configuración 
...
core/
: La aplicación principal del negocio. Contiene:


models.py
: Modelos de datos para Categorías, Estatus, Publicaciones, Galerías, Archivos, Comentarios y Perfil.


views.py
: Lógica de control y renderizado para la sección pública y el panel del dashboard.
...
middleware.py
: Lógica del registro de logs HTTP en formato JSON.


templates/
: Plantillas HTML del proyecto utilizando el motor de plantillas de Django.


static/
: Archivos estáticos del frontend (Hojas de estilo CSS personalizadas, scripts JS para AJAX y librerías externas como Bootstrap).


media/
: Almacenamiento de archivos subidos dinámicamente (imágenes destacadas, fotos de perfil, galerías y adjuntos descargables).


logs/
: Carpeta donde se guarda el historial de actividad HTTP en JSON.
